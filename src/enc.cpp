...
zigzag_block zigzag_encode(block curr_block) {
    ...
    for (int i = 0; i < 7; i++) {
        if (i == 0) res[count++] = curr_block[i][i];

        if (i > 0) {
            int index = 0;

            for (int k = i, col = 0; k >= 0; k--, col++) tmp_store[index++] = curr_block[k][col];
    ...
    for (int i = 0; i < 8; i++) {
        int index = 0;
        for (int k = i, col = 7; k < 8; k++, col--) tmp_store[index++] = curr_block[k][col];
